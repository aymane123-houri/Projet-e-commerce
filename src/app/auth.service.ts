...
export class AuthService {
  ...
  constructor(private http :HttpClient,  private router: Router){
    ...
    this.http.get<User[]>('http://localhost:8080/users').subscribe((data)=>{this.users = data});
  ...
  createtUser(user : User,adminCode?: string) : Observable<User> {
    ...
    return this.http.post<User>('http://localhost:8080/users/post',data);
  }
...
*/
  ...
  deleteUser(userId: number): Observable<void> {
    const url = `http://localhost:8080/users/delete/${userId}`;
